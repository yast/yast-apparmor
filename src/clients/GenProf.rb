...
module Yast
  class GenProfClient < Client
    def main
      ...
      @agent_data = Convert.to_map(SCR.Read(path(".genprof")))
      ...
      SCR.Write(path(".genprof"), @agent_data)

      # open up the initial form window...
      Wizard.CreateDialog
      begin
        @agent_data = Convert.to_map(SCR.Read(path(".genprof")))
        Builtins.y2internal("%1", @agent_data)
        ...
        if @type2 == "wizard"
          ...
          SCR.Write(path(".genprof"), @answers)
          ...
          @written = SCR.Write(path(".genprof"), @answers)
          ...
          begin
            ...
            elsif @ret == :cancel || @ret == :abort || @ret == :back
          ...
          end until @ret == :next || @ret == :cancel || @ret == :abort || @ret == :back
